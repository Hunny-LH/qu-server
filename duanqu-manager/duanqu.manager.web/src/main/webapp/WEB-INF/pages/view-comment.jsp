...
<script type="text/javascript">
	...
	function reply(cid,replyUid,prientId,rootId){
		...
		var str="user.do?method=enterComment&uid=${vo.uid}&cid="+cid+"&replyUid="+replyUid+"&parentId="+prientId+"&rootId="+rootId;
   	...
   	function query(index){
		...
		form1.action="user.do?method=mjCommentList";
		form1.submit();	
	}
   function deleteComment(cid,id,rootId){
	   form1.action="user.do?method=deleteComment&cid="+cid+"&id="+id+"&rootId="+rootId;
	   form1.submit();	
  }
   ...
   {
	   DemoController.updateComment(${vo.uid},function(data){
			 alert(data);
...
<input name="" type="button"  value="已查看该马甲下所有评论" class="button" onClick="updateComment()">
<div id="Main">
<div class="table-list">
 <form  method="post" name="form1">
 
 <input type="hidden" name="uid" value="${vo.uid}" ></input>
    ...
    <thead>
              ...
              <tbody>
                ...
                <tr>
				  ...
				  <a href="###" onClick="deleteComment('${item.cid}','${item.id}','${item.rootId}')">删除</a>
				  </td>
                </tr>
              </c:forEach>
